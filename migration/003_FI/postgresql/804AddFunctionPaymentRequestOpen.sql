...
BEGIN
	...
	SELECT prl.AD_Client_ID, prl.AD_Org_ID, SUM(prl.PayAmt) PayAmt,pr.C_Currency_ID,pr.DateDoc--, p.PayAmt PaymenAmt
		...
		LEFT JOIN C_PaySelectionLine psl ON (psl.LVE_PaymentRequestLine_ID = prl.LVE_PaymentRequestLine_ID)
	...
	IF (p_RecordPaySchedule_ID > 0) THEN --   if not valid = lists invoice amount
	  ...
	  FOR s IN 
		  ...
		  UNION ALL 
		  SELECT  LVE_OrderPrepaySchedule_ID AS RecordPaySchedule_ID, DueAmt,DueDate
