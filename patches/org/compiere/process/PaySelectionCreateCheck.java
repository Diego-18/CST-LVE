...
//		Try to find one
		for (int i = 0; i < m_list.size(); i++)
		{
			MPaySelectionCheck check = (MPaySelectionCheck)m_list.get(i);
			//	Add to existing
			/** 2014-02-05 Carlos Parada */
			if (line.get_ValueAsInt("LVE_PaymentRequestLine_ID") != 0){
				...
				if (prl.getC_Invoice_ID()!=0){
					if (new Query(getCtx(),MPaySelectionLine.Table_Name,  "C_PaySelectionCheck_ID = ? "
							+ "AND EXISTS(SELECT 1 FROM C_PaySelectionCheck psc WHERE psc.C_PaySelectionCheck_ID=C_PaySelectionLine.C_PaySelectionCheck_ID AND COALESCE(psc.C_BP_BankAccount_ID,0)=?) "
							+ "AND EXISTS(SELECT 1 FROM C_Invoice i WHERE i.C_Invoice_ID = C_PaySelectionLine.C_Invoice_ID AND i.C_BPartner_ID =?)", 
							prl.get_TrxName()).setParameters(check.getC_PaySelectionCheck_ID(),prl.getC_BP_BankAccount_ID(),prl.getC_BPartner_ID()).match()){
						appendCheck(check, line ,prl);
						return;
					}
				}else if (prl.getC_Order_ID()!=0){
					if (new Query(getCtx(),MPaySelectionLine.Table_Name,  "C_PaySelectionCheck_ID = ? "
							+ "AND EXISTS(SELECT 1 FROM C_PaySelectionCheck psc WHERE psc.C_PaySelectionCheck_ID=C_PaySelectionLine.C_PaySelectionCheck_ID AND COALESCE(psc.C_BP_BankAccount_ID,0)=?) "
							+ "AND EXISTS(SELECT 1 FROM C_Order o WHERE o.C_Order_ID = C_PaySelectionLine.C_Order_ID AND o.C_Order_ID =?)", 
							prl.get_TrxName()).setParameters(check.getC_PaySelectionCheck_ID(),prl.getC_BP_BankAccount_ID(),prl.getC_Order_ID()).match()){
						appendCheck(check, line ,prl);
						return;
					}
					...
					if (new Query(getCtx(),MPaySelectionLine.Table_Name,  "C_PaySelectionCheck_ID = ? "
							+ "AND EXISTS(SELECT 1 FROM C_PaySelectionCheck psc WHERE psc.C_PaySelectionCheck_ID=C_PaySelectionLine.C_PaySelectionCheck_ID AND COALESCE(psc.C_BP_BankAccount_ID,0)=?) "
							+ "AND EXISTS(SELECT 1 FROM GL_JournalLine jl WHERE jl.GL_JournalLine_ID = C_PaySelectionLine.GL_JournalLine_ID AND jl.C_BPartner_ID =?)",
							prl.get_TrxName()).setParameters(check.getC_PaySelectionCheck_ID(),prl.getC_BP_BankAccount_ID(),prl.getC_BPartner_ID()).match()){
						appendCheck(check, line ,prl);
						return;
					}
				}else {
					MLVEPaymentRequest pr = null;
					if (prl.getLVE_PaymentRequest_ID()!=0)
						pr = new MLVEPaymentRequest(getCtx(), prl.getLVE_PaymentRequest_ID(), prl.get_TrxName());
					if (pr!=null){
						if (new Query(getCtx(),MPaySelectionLine.Table_Name,  "C_BPartner_ID=? "
																			+ "AND C_PaySelectionCheck_ID = ? "
																			+ "AND EXISTS(SELECT 1 FROM C_PaySelectionCheck psc WHERE psc.C_PaySelectionCheck_ID=C_PaySelectionLine.C_PaySelectionCheck_ID AND COALESCE(psc.C_BP_BankAccount_ID,0)=?) "
																			+ "AND EXISTS(SELECT 1 FROM LVE_PaymentRequest pr WHERE pr.LVE_PaymentRequest_ID = C_PaySelectionLine.LVE_PaymentRequest_ID AND COALESCE(C_Charge_ID,0) = ? )", 
																			prl.get_TrxName()).setParameters(prl.getC_BPartner_ID(),check.getC_PaySelectionCheck_ID(),prl.getC_BP_BankAccount_ID(),pr.getC_Charge_ID()).match()){
							appendCheck(check, line ,prl);
							return;
						}
					}
			...
			else if (check.getC_BPartner_ID() == line.getInvoice().getC_BPartner_ID())
			{
				appendCheck(check, line, null);
				return;
			}
			/** End Carlos Parada*/ 
		}
	...
	/**
	 * 
	 * @author <a href="mailto:carlosaparadam@gmail.com">Carlos Parada</a> 5/2/2015, 19:50:08
	 * @param check
	 * @param line
	 * @return
	 * @return boolean
	 */
	private boolean appendCheck(MPaySelectionCheck check,MPaySelectionLine line,MLVEPaymentRequestLine prl){
		check.addLine(line);
		if (prl != null)
			check.setC_BP_BankAccount_ID(prl.getC_BP_BankAccount_ID());
