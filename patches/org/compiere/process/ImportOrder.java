...
public class ImportOrder extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql = new StringBuffer ("UPDATE I_Order "
			  ...
			  + " AD_Org_ID = COALESCE ("+/*"AD_Org_ID"*/"AD_OrgTrx_ID,").append (m_AD_Org_ID).append ("),"
		...
		//	Price List
		//Carlos Parada Price List By Name
		sql = new StringBuffer ("UPDATE I_Order o "
			  + "SET M_PriceList_ID=(SELECT MAX(M_PriceList_ID) FROM M_PriceList p WHERE p.Name = o.PriceListName AND o.AD_Client_ID=p.AD_Client_ID) "
		 ...
		 * Move code for location BPartner 
		 */
		//	Set Country
		/**
		sql = new StringBuffer ("UPDATE I_Order o "
		...
		**/
		...
		sql = new StringBuffer ("UPDATE I_Order o "
			  + "SET (BillTo_ID,C_BPartner_Location_ID)=(SELECT C_BPartner_Location_ID,C_BPartner_Location_ID"
			  + " FROM  C_BPartner_Location bpl INNER JOIN C_BPartner bp ON (bp.C_BPartner_ID=bpl.C_BPartner_ID) " 
			  + " INNER JOIN C_Location l ON (bpl.C_Location_ID=l.C_Location_ID)"
			  + " LEFT JOIN C_Country c ON (c.C_Country_ID=l.C_Country_ID)"
			  + " ,C_Region r "
			  + " WHERE r.C_Country_ID = l.C_Country_ID AND bpl.AD_Client_ID=o.AD_Client_ID"
			  + " AND DUMP(o.Address1)=DUMP(l.Address1) AND DUMP(o.Address2)=DUMP(l.Address2)"
			  + " AND DUMP(o.City)=DUMP(l.City) AND DUMP(o.Postal)=DUMP(l.Postal)"
			  + " AND r.Name=o.RegionName AND c.CountryCode=o.CountryCode AND bp.Value=o.BPartnerValue ) "
			  + "WHERE C_BPartner_ID IS NOT NULL AND C_BPartner_Location_ID IS NULL"
			  + " AND I_IsImported='N'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		sql = new StringBuffer ("SELECT * FROM I_Order "
			...
			.append(" ORDER BY C_BPartner_ID,DocumentNo, BillTo_ID, C_BPartner_Location_ID, I_Order_ID");
		try
		{
			PreparedStatement pstmt = DB.prepareStatement (sql.toString(), get_TrxName());
			ResultSet rs = pstmt.executeQuery ();
