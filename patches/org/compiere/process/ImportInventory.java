...
public class ImportInventory extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		/** Carlos Parada 2013-09-18 Do Not Raise Error */
		/*
		sql = new StringBuffer ("UPDATE I_Inventory "
		...
		/** Carlos Parada 2013-09-18 Set WareHouse From Value */
		//	Set M_Warehouse_ID
		/*
		sql = new StringBuffer ("UPDATE I_Inventory i "
			...
			*/
		sql = new StringBuffer ("UPDATE I_Inventory i "
				+ "SET M_Warehouse_ID=(SELECT M_Warehouse_ID FROM M_Warehouse w WHERE i.WarehouseValue=w.Value " 
				+" AND i.AD_Client_ID=w.AD_Client_ID) "
				+ "WHERE M_Warehouse_ID IS NULL"
				+ " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate (sql.toString (), get_TrxName());
		
		log.fine("Set Warehouse Value=" + no);
		
		//Moved for Warehouse Filter
		//		Location
		sql = new StringBuffer ("UPDATE I_Inventory i "
			+ "SET M_Locator_ID=(SELECT MAX(M_Locator_ID) FROM M_Locator l"
			+ " WHERE i.LocatorValue=l.Value AND i.AD_Client_ID=l.AD_Client_ID AND i.M_Warehouse_ID=l.M_Warehouse_ID) "
			+ "WHERE M_Locator_ID IS NULL AND LocatorValue IS NOT NULL AND M_Warehouse_ID Is Not Null"
			+ " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate (sql.toString (), get_TrxName());
		log.fine("Set Locator from Value =" + no);
		sql = new StringBuffer ("UPDATE I_Inventory i "
			+ "SET M_Locator_ID=(SELECT MAX(M_Locator_ID) FROM M_Locator l"
			+ " WHERE i.X=l.X AND i.Y=l.Y AND i.Z=l.Z AND i.AD_Client_ID=l.AD_Client_ID AND i.M_Warehouse_ID=l.M_Warehouse_ID AND i.LocatorValue=l.Value) "
			+ "WHERE M_Locator_ID IS NULL AND X IS NOT NULL AND Y IS NOT NULL AND Z IS NOT NULL AND M_Warehouse_ID Is Not Null"
			+ " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate (sql.toString (), get_TrxName());
		log.fine("Set Locator from X,Y,Z =" + no);
		...
		try
		{
			...
			while (rs.next())
			{
				...
				/** Carlos Parada 2013-09-18 Create Locator if Don't Exists*/
				if (imp.getM_Locator_ID()==0)
				{
					imp.setM_Locator_ID(getLocator(getCtx(), imp.getM_Warehouse_ID(), imp.getLocatorValue(), imp.getX(), imp.getY(), imp.getZ(), get_TrxName()));
					if (imp.getM_Locator_ID()==0){
						MLocator locator = new MLocator(new MWarehouse(getCtx(), imp.getM_Warehouse_ID(), get_TrxName()), imp.getLocatorValue());
						locator.setX(imp.getX());
						locator.setY(imp.getY());
						locator.setZ(imp.getZ());
						locator.save(get_TrxName());
	...
	public int getLocator (Properties ctx, int M_Warehouse_ID, String Value,
			 String X, String Y, String Z,String trxname)
		 {
			int retValue=0;
			String sql = "SELECT M_Locator_ID FROM M_Locator WHERE IsActive = 'Y' AND M_Warehouse_ID=? AND COALESCE(X,'')=? AND COALESCE(Y,'')=? AND COALESCE(Z,'')=? AND Value=?";
			...
			try
			{
				...
				if (rs.next())
					retValue = rs.getInt("M_Locator_ID");
			}
