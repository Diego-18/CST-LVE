...
public class ImportProduct extends SvrProcess implements ImportProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		/**
		 * Update 2013-08-02 Carlos Parada
		 * Set Tax Category
		 */
		//		Set Product Category
		sql = new StringBuffer ("UPDATE I_Product "
			+ "SET TaxCategory_Name=(SELECT MAX(Name) FROM C_TaxCategory"
			+ " WHERE IsDefault='Y' AND AD_Client_ID=").append(m_AD_Client_ID).append(") "
			+ "WHERE TaxCategory_Name IS NULL AND C_TaxCategory_ID IS NULL"
			+ " AND M_Product_ID IS NULL"	//	set category only if product not found 
			+ " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.fine("Set Tax Category Default Value=" + no);
		//
		sql = new StringBuffer ("UPDATE I_Product i "
			+ "SET C_TaxCategory_ID=(SELECT C_TaxCategory_ID FROM C_TaxCategory c"
			+ " WHERE i.TaxCategory_Name=c.Name AND i.AD_Client_ID=c.AD_Client_ID) "
			+ "WHERE TaxCategory_Name IS NOT NULL AND C_TaxCategory_ID IS NULL"
			+ " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		/** 2013-08-07 Carlos Parada
		 * Add Error Message for Invalid Tax Category
		 * **/
		//	Invalid Tax Category
		sql = new StringBuffer ("UPDATE I_Product "
			+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid TaxCategory,' "
...
*/
			...
			while (rs.next())
			{
				...
				if (newProduct)			//	Insert new Product
				{
					...
					/**
					 * Updated 2013-08-02 Carlos Parada
					 * Set Tax Category for Import And TariffCode
					 */
					//product.setC_TaxCategory_ID(C_TaxCategory_ID);
					product.setC_TaxCategory_ID(imp.get_ValueAsInt("C_TaxCategory_ID"));
					product.set_ValueOfColumn("TariffCode", imp.get_Value("TariffCode"));
					product.setM_AttributeSet_ID(imp.get_ValueAsInt("M_AttributeSet_ID"));
					
					//Set Attribute Instance
					if (imp.get_ValueAsInt("M_AttributeValue_ID")!=0 && product.getM_AttributeSet_ID()!=0)
					{
						MAttributeValue val = new MAttributeValue(getCtx(), imp.get_ValueAsInt("M_AttributeValue_ID"), get_TrxName());
						MAttributeSetInstance atsi = new MAttributeSetInstance(getCtx(), -1, get_TrxName());
						atsi.setM_AttributeSet_ID(product.getM_AttributeSet_ID());
						atsi.setDescription(val.getDescription());
						atsi.save();
						
						MAttributeInstance ati =new MAttributeInstance(getCtx(), val.getM_Attribute_ID(), atsi.getM_AttributeSetInstance_ID(), val.getM_AttributeValue_ID(),val.getValue(), get_TrxName());
						ati.save();
						product.setM_AttributeSetInstance_ID(atsi.getM_AttributeSetInstance_ID());
				...
				{
					/**
					 * Updated 2013-09-18 Carlos Parada
					 * Set Tax Category for Import And TariffCode
					 */
					String sqlt = "UPDATE M_PRODUCT "
						+ "SET (Value,Name,Description,DocumentNote,Help,"
						+ "UPC,SKU,C_UOM_ID,M_Product_Category_ID,Classification,ProductType,"
						+ "Volume,Weight,ShelfWidth,ShelfHeight,ShelfDepth,UnitsPerPallet,"
						+ "Discontinued,DiscontinuedBy, DiscontinuedAt, Updated,UpdatedBy,C_TaxCategory_ID,TariffCode)= "
						+ "(SELECT Value,Name,Description,DocumentNote,Help,"
						+ "UPC,SKU,C_UOM_ID,M_Product_Category_ID,Classification,ProductType,"
						+ "Volume,Weight,ShelfWidth,ShelfHeight,ShelfDepth,UnitsPerPallet,"
						+ "Discontinued,DiscontinuedBy, DiscontinuedAt, SysDate,UpdatedBy,C_TaxCategory_ID,TariffCode"
					...
					/** Carlos Parada Set Attribute Instance**/
					MProduct product = new MProduct(getCtx(),M_Product_ID,get_TrxName());
					if (product.getM_AttributeSet_ID()!=imp.get_ValueAsInt("M_AttributeSet_ID"))
					{
						// AttributeSetInstance reset
						MAttributeSetInstance asi = new MAttributeSetInstance(getCtx(), product.getM_AttributeSetInstance_ID(), get_TrxName());
						
						product.setM_AttributeSetInstance_ID(0);
