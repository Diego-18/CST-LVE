...
implements ImportProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		//	Yamel Senih 23/07/2013 10:58 Set Org
		//	Existing Org ? Match Value
		sql = new StringBuffer ("UPDATE I_BPartner i "
				+ "SET AD_Org_ID=(SELECT AD_Org_ID FROM AD_Org o"
				+ " WHERE i.OrgValue=o.Value AND o.AD_Client_ID=i.AD_Client_ID) "
				+ "WHERE i.OrgValue IS NOT NULL"
				+ " AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		log.fine("Found Org=" + no);
		
		//	Carlos Parada 
		//	Existing Bank ? Match BankName
		
		sql = new StringBuffer ("UPDATE I_BPartner i "
				+ "SET C_Bank_ID=(SELECT Max(C_Bank_ID) FROM C_Bank b"
				+ " WHERE i.BankName=b.Name AND b.AD_Client_ID=i.AD_Client_ID) "
				+ "WHERE i.BankName IS NOT NULL"
				+ " AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		log.fine("Found Bank=" + no);
...
//		Existing Location ? Exact Match
		...
		/** 2013-09-18 Carlos Parada Locate BankAccounts*/
		//	Existing BankAccounts ? Match AccountNo Or CreditCardNumber
		sql = new StringBuffer ("UPDATE I_BPartner i "
				+ "SET C_BP_BankAccount_ID=(SELECT Max(C_BP_BankAccount_ID) FROM C_BP_BankAccount ba "
				+ "Left JOIN C_Bank b ON (b.C_Bank_ID=ba.C_Bank_ID) "
				+ " WHERE (i.IsACH='Y' AND b.Name=i.BankName AND i.AccountNo=ba.AccountNo AND i.BankAccountType=ba.BankAccountType AND i.C_BPartner_ID=ba.C_BPartner_ID ) "
				+ " OR (i.IsACH='N' AND i.CreditCardType=ba.CreditCardType AND i.CreditCardNumber=ba.CreditCardNumber AND i.CreditCardExpYY=ba.CreditCardExpYY AND i.CreditCardExpMM=ba.CreditCardExpMM)"
				+ "							) "
				+ "WHERE C_BPartner_ID IS NOT NULL AND ((i.IsACH='Y' AND AccountNo Is Not Null) OR (i.IsACH='N' AND CreditCardNumber Is Not Null))"
				+ " AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		log.fine("Found BankAccount=" + no);
			...
			{	
				...
				if ( ! New_BPValue.equals(Old_BPValue)) {
					...
					if (impBP.getC_BPartner_ID() == 0)	//	Insert new BPartner
					{
						...
						/**Carlos Parada 2013-09-18 Set Person Type*/
						
						bp.set_ValueOfColumn("LVE_PersonType_ID", impBP.get_Value("LVE_PersonType_ID"));
					...
					/**
					 * Update With Carlos Parada for Location BPartner
					 */
				...
				if (impBP.getAD_User_ID() != 0)
				{
					...
					/**Carlos Parada 2013-09-18 Add Employee Fields*/
					/*
					if (impBP.get_Value("Degree") != null)
						user.set_ValueOfColumn("Degree",impBP.get_Value("Degree"));
					if (impBP.get_Value("Gender") != null)
						user.set_ValueOfColumn("Gender",impBP.get_Value("Gender"));
					if (impBP.get_Value("IsDepending") != null)
						user.set_ValueOfColumn("IsDepending",impBP.get_Value("IsDepending"));
					if (impBP.get_Value("IsScholarship") != null)
						user.set_ValueOfColumn("IsScholarship",impBP.get_Value("IsScholarship"));
					if (impBP.get_Value("NationalCode") != null)
						user.set_ValueOfColumn("NationalCode",impBP.get_Value("NationalCode"));
					if (impBP.get_Value("Relationship") != null)
						user.set_ValueOfColumn("Relationship",impBP.get_Value("Relationship"));
					if (impBP.get_Value("Turn") != null)
						user.set_ValueOfColumn("Turn",impBP.get_Value("Turn"));
					*/
					/** End Carlos Parada */
				...
				/**
				 * Carlos Parada Add BPBankAccounts
				 */
				//	****	Create/Update BPartner BankAccount	****
				MBPBankAccount bacct = null;
				if (impBP.get_ValueAsInt("C_BP_BankAccount_ID") != 0)
				{
				bacct = new MBPBankAccount (getCtx(), impBP.get_ValueAsInt("C_BP_BankAccount_ID"), get_TrxName());
				
				bacct.setIsACH(impBP.get_ValueAsBoolean("IsACH"));
				bacct.setC_Bank_ID(impBP.get_ValueAsInt("C_Bank_ID"));
				bacct.setAccountNo(impBP.get_ValueAsString("AccountNo"));
				bacct.setA_City(impBP.get_ValueAsString("A_City"));
				bacct.setA_Country(impBP.get_ValueAsString("A_Country"));
				bacct.setA_EMail(impBP.get_ValueAsString("A_EMail"));
				bacct.setA_Ident_DL(impBP.get_ValueAsString("A_Ident_DL"));
				bacct.setA_Ident_SSN(impBP.get_ValueAsString("A_Ident_SSN"));
				bacct.setA_Name(impBP.get_ValueAsString("A_Name"));
				bacct.setA_State(impBP.get_ValueAsString("A_State"));
				bacct.setA_Street(impBP.get_ValueAsString("A_Street"));
				bacct.setA_Zip(impBP.get_ValueAsString("A_Zip"));
				bacct.setCreditCardExpMM(impBP.get_ValueAsInt("CreditCardExpMM"));
				bacct.setCreditCardExpYY(impBP.get_ValueAsInt("CreditCardExpYY"));
				bacct.setCreditCardNumber(impBP.get_ValueAsString("CreditCardNumber"));
				bacct.setCreditCardVV(impBP.get_ValueAsString("CreditCardVV"));
				bacct.setRoutingNo(impBP.get_ValueAsString("RoutingNo"));
				
				if (!impBP.get_ValueAsString("CreditCardType").equals(""))
					bacct.setCreditCardType(impBP.get_ValueAsString("CreditCardType"));
				if (!impBP.get_ValueAsString("BankAccountType").equals(""))
					bacct.setBankAccountType(impBP.get_ValueAsString("BankAccountType"));
				if (!impBP.get_ValueAsString("BPBankAcctUse").equals(""))
					bacct.setBPBankAcctUse(impBP.get_ValueAsString("BPBankAcctUse"));
				
				if (bacct.getC_BPartner_ID() == 0)
					bacct.setC_BPartner_ID(bp.getC_BPartner_ID());
				else if (bacct.getC_BPartner_ID() != bp.getC_BPartner_ID())
				{
					rollback();
					noInsert--;
					sql = new StringBuffer ("UPDATE I_BPartner i "
							+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
					.append("'BP of BankAccount <> BP, ' ")
					.append("WHERE I_BPartner_ID=").append(impBP.getI_BPartner_ID());
					DB.executeUpdateEx(sql.toString(), get_TrxName());
					continue;
				}
				ModelValidationEngine.get().fireImportValidate(this, impBP, bacct, ImportValidator.TIMING_AFTER_IMPORT);
				if (bacct.save())
				{
					log.finest("Update BP BankAcount - " + bacct.getC_BP_BankAccount_ID());
				}
				else
				{
					rollback();
					...
					sql = new StringBuffer ("UPDATE I_BPartner i "
							+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
					.append("'Cannot Update BP BankAcount, ' ")
					.append("WHERE I_BPartner_ID=").append(impBP.getI_BPartner_ID());
					DB.executeUpdateEx(sql.toString(), get_TrxName());
					continue;
				}
				...
				if ((impBP.get_ValueAsBoolean("IsACH") && 
						impBP.get_ValueAsInt("C_Bank_ID")!=0 &&
							impBP.get_Value("AccountNo")!=null)
					||
					(!impBP.get_ValueAsBoolean("IsACH") && 
							impBP.get_Value("CreditCardNumber")!=null)
				)
				{
					bacct = new MBPBankAccount (getCtx(), 0, get_TrxName());
					...
					if (bacct.save())
					{
						log.finest("Insert BP BankAcconunt - " + bacct.getC_BP_BankAccount_ID());
						impBP.set_ValueOfColumn("C_BP_BankAccount_ID",bacct.getC_BP_BankAccount_ID());
					}
					else
					{
						rollback();
						...
						sql = new StringBuffer ("UPDATE I_BPartner i "
								+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||")
						.append("'Cannot Insert BankAcount, ' ")
						.append("WHERE I_BPartner_ID=").append(impBP.getI_BPartner_ID());
						DB.executeUpdateEx(sql.toString(), get_TrxName());
						continue;
					}
