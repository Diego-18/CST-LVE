...
//		Add lines from Patches of LVE-ADempiere and had no comments	
		String m_ClientName = "(" + MClient.get(ctx).getValue() + ")";
...
//    	to.setName(suggestedName);
    	...
    	if(to.getName().contains(m_ClientName))
			to.setName(to.getName() + " ");		//	unique name
		else
			to.setName(to.getName().trim() + " " + m_ClientName);
//		End Dixon Martinez
