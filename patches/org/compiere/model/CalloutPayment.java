...
public class CalloutPayment extends CalloutEngine
{
	 ...
	 * Add Order Pre-Payment Schedule
	 */
...
//		Add change created by Yamel Senih
		...
		if (Env.getContextAsInt (ctx, WindowNo, Env.TAB_INFO, "C_Order_ID") == C_Order_ID.intValue ()
			&& Env.getContextAsInt (ctx, WindowNo, Env.TAB_INFO, "LVE_OrderPrePaySchedule_ID") != 0)
		{
			LVE_OrderPrePaySchedule_ID = Env.getContextAsInt (ctx, WindowNo, Env.TAB_INFO, "LVE_OrderPrePaySchedule_ID");
		}
...
//			+ "FROM C_Order WHERE C_Order_ID=?"; // #1
		...
		//
		String sql = "SELECT COALESCE(Bill_BPartner_ID, C_BPartner_ID) as C_BPartner_ID "	//	1
			+ ", C_Currency_ID "															//	2
			+ ", orderOpen(C_Order_ID, ?) " 												//	3 #1
			+ ", orderDiscount(C_Order_ID,?,?), IsSOTrx " 									//	4..5 #2/3
			+ "FROM C_Order WHERE C_Order_ID=?"; 											//	#4
...
//			pstmt.setInt (1, C_Order_ID.intValue ());
			...
			pstmt.setTimestamp (2, ts);
			pstmt.setInt (3, LVE_OrderPrePaySchedule_ID);
			pstmt.setInt (4, C_Order_ID.intValue ());
//			End Dixon Martinez
...
//				mTab.setValue ("PayAmt", GrandTotal);
				...
				BigDecimal OrderOpen = rs.getBigDecimal (3); // Set Order
				// OPen Amount
				if (OrderOpen == null)
					OrderOpen = Env.ZERO;
				BigDecimal DiscountAmt = rs.getBigDecimal (4); // Set Discount
				...
				if (DiscountAmt == null)
					DiscountAmt = Env.ZERO;
				mTab.setValue ("PayAmt", OrderOpen.subtract (DiscountAmt));
				mTab.setValue ("DiscountAmt", DiscountAmt);
				// reset as dependent fields get reset
				Env.setContext (ctx, WindowNo, "C_Order_ID", C_Order_ID
					.toString ());
