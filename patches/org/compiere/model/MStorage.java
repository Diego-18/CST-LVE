...
//		}
		//		
		//2014-06-23 Carlos Parada Set Attribute Instance When Attribute Set is Zero And Attribute Set Instance is Zero
		if (prd.getM_AttributeSet_ID() == 0 && M_AttributeSetInstance_ID ==0 ) {
			// Product doesn't manage attribute set, always reserved with 0
			reservationAttributeSetInstance_ID = 0;
		}
//		End Dixon Martinez
	...
	/**************************************************************************
	 *  Carlos Parada get Locator And Instance Attribute 
	 * 	Get Location with highest Locator Priority and a sufficient OnHand Qty
	 * 	@param M_Warehouse_ID warehouse
	 * 	@param M_Product_ID product
	 * 	@param M_AttributeSetInstance_ID asi
	 ...
	 * 	@return id
	 */
	public static HashMap<Integer,Integer> getM_Locator_ID (int M_Warehouse_ID, 
		int M_Product_ID , BigDecimal Qty,
		String trxName)
	{
		HashMap<Integer,Integer> m_Locator = new HashMap<Integer,Integer>();
		int M_Locator_ID = 0;
		int firstM_Locator_ID = 0;
		String sql = "SELECT s.M_Locator_ID, s.QtyOnHand, s.M_AttributeSetInstance_ID "
			+ "FROM M_Storage s"
			+ " INNER JOIN M_Locator l ON (s.M_Locator_ID=l.M_Locator_ID)"
			+ " INNER JOIN M_Product p ON (s.M_Product_ID=p.M_Product_ID)"
			+ " LEFT OUTER JOIN M_AttributeSet mas ON (p.M_AttributeSet_ID=mas.M_AttributeSet_ID) "
			+ "WHERE l.M_Warehouse_ID=?"
			+ " AND s.M_Product_ID=?"
			+ " AND (mas.IsInstanceAttribute IS NULL OR mas.IsInstanceAttribute='N' OR s.M_AttributeSetInstance_ID = (SELECT max(ss.M_AttributeSetInstance_ID) FROM M_Storage ss WHERE ss.M_Product_ID = s.M_Product_ID " 
			+ " AND ss.M_Locator_ID=s.M_Locator_ID AND QtyOnHand <> 0)) "  
			+ " AND l.IsActive='Y' "
			+ "ORDER BY l.PriorityNo DESC, s.QtyOnHand DESC";
		...
		try
		{
			...
			while (rs.next())
			{
				...
				{
					M_Locator_ID = rs.getInt(1);
					m_Locator.put(M_Locator_ID, rs.getInt(3));
					break;
				}
				if (firstM_Locator_ID == 0){
					firstM_Locator_ID = rs.getInt(1);
					m_Locator.put(firstM_Locator_ID, rs.getInt(3));
				}
			}
		}
		catch (SQLException ex)
