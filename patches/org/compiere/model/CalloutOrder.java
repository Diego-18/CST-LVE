...
//			Dixon Martinez 2015-08-25 
//			Add variable currency
...
//			End Dixon Martinez
			...
			if (rs.next())
			{
				...
				//	Currency
				currencyID = new Integer(rs.getInt(3));
				mTab.setValue("C_Currency_ID", currencyID);
				//	PriceList Version
				Env.setContext(ctx, WindowNo, "M_PriceList_Version_ID", rs.getInt(5));
			}
//			Dixon Martinez 2015-08-25 
//			If you are unable version of price list you must add the currency of the price list selected to indicate the conversion of currency
			if(currencyID == -1) {
				sql = "SELECT pl.C_Currency_ID FROM M_PriceList pl WHERE pl.M_PriceList_ID= ?";
				currencyID = DB.getSQLValue(null, sql, M_PriceList_ID);
				mTab.setValue("C_Currency_ID", currencyID);
			}
//			End Dixon Martinez
	...
	/**
	 * Add Support to Order Pre-Payment Schedule
	 * @author <a href="mailto:yamelsenih@gmail.com">Yamel Senih</a> 16/03/2013, 00:48:54
	 * @param ctx
	 * @param WindowNo
	 * @param mTab
	 * @param mField
	...
	public String paymentTerm (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		boolean valid = pt.applyToOrder (C_Order_ID);
