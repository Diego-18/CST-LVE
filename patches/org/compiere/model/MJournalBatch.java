		...
		if (DOCSTATUS_Closed.equals(getDocStatus())
				...
				|| DOCSTATUS_NotApproved.equals(getDocStatus()) )
			{
				...
				for (int i = 0; i < journals.length; i++)
				{
					...
					journal.setTotalCr(Env.ZERO);
					journal.setTotalDr(Env.ZERO);
					journal.setDescription(getDescription()+ " " + Msg.getMsg(getCtx(), "Voided" ));
					journal.setControlAmt(Env.ZERO);
					journal.setPosted(true);
					
					
					MJournalLine[] journalLineArray = journal.getLines(true);
					
					for (MJournalLine journalLine : journalLineArray) {
						journalLine.setAmtAcctCr(Env.ZERO);
						journalLine.setAmtAcctDr(Env.ZERO);
						journalLine.setAmtSourceCr(Env.ZERO);
						journalLine.setAmtSourceDr(Env.ZERO);
						journalLine.setQty(Env.ZERO);
						journalLine.setA_Processed(true);
						journalLine.setDescription(journalLine.getDescription() +" " + Msg.getMsg(getCtx(), "Voided") );
						journalLine.saveEx();
						
					}
					journal.saveEx();
					
					journal.setDocStatus(X_GL_JournalBatch.DOCACTION_Void);
					journal.setProcessed(true);
					journal.saveEx();
				}
			}
