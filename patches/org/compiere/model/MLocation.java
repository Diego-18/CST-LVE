...
public class MLocation extends X_C_Location implements Comparator
{
	 ...
	 */
	private String parseCRP (MCountry c, String cusInStr)
	{
		if (c == null)
		...
		//	Yamel Senih 2014-04-22, 17:28:00
		//	Add Custom Display Sequence own Customer
		if(cusInStr != null
				&& cusInStr.length() > 0)
			inStr = cusInStr;
		else
			inStr = local ? c.getDisplaySequenceLocal() : c.getDisplaySequence();
		//	End Yamel Senih
	 ...
	 *  @return String
	 */
	public String toStringCR(String cusInStr)
	{
		StringBuffer retStr = new StringBuffer();
		if (isAddressLinesReverse())
		{
...
//			retStr.append(parseCRP (getCountry()));
			retStr.append(parseCRP (getCountry(), cusInStr));
		...
		else
		{
			...
			if (getAddress4() != null && getAddress4().length() > 0)
				retStr.append("\n").append(getAddress4());
			//	City, Region, Postal
			retStr.append("\n").append(parseCRP (getCountry(), cusInStr));
			//	Add Country would come here
		}
		return retStr.toString();
	}	//	toStringCR
