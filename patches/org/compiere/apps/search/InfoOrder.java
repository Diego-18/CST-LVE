...
//	private static String s_From = " C_Order o";
	private static String s_From = " LVE_Order_v o";
	/** Order Clause             */
...
//		Add lines from Patches of LVE-ADempiere and had no comments		
		list.add(new Info_Column(Msg.translate(Env.getCtx(), "OpenAmt"), "orderOpen(o.C_Order_ID, o.LVE_OrderPrePaySchedule_ID)",  BigDecimal.class));
		list.add(new Info_Column(Msg.translate(Env.getCtx(), "DueDate"), "o.DueDate",  Timestamp.class));
//		End Dixon Martinez
...
//		list.add(new Info_Column(Msg.translate(Env.getCtx(), "DocStatus"), "o.docstatus", String.class));
		list.add(new Info_Column(Msg.translate(Env.getCtx(), "DocStatus"), "(Select COALESCE(rft.Name,rf.Name) rName "
				+ "From AD_Ref_List rf LEFT JOIN AD_Ref_List_Trl rft ON ( rf.AD_Ref_List_ID=rft.AD_Ref_List_ID AND rft.AD_Language = '" +Env.getContext(Env.getCtx(), "#AD_Language") + "' ) "
				+ "WHERE rf.AD_Reference_ID=131 AND rf.Value = o.DocStatus )", String.class));

//		list.add(new Info_Column(Msg.translate(Env.getCtx(), "LVE_OrderPrePaySchedule_ID"), 
//				"(SELECT DueDate || '-' || DueAmt  FROM LVE_OrderPrePaySchedule opps WHERE opps.LVE_OrderPrePayschedule_ID = o.LVE_OrderPrePayschedule_ID) ",
//				KeyNamePair.class, "o.LVE_OrderPrePaySchedule_ID"));
//		End Dixon Martinez
		//
		s_Layout = new Info_Column[list.size()];
		list.toArray(s_Layout);
		
		INDEX_PAYSCHEDULE =  s_Layout.length - 1;
		//
		return s_Layout;
	}   //  getTableLayout
	...
	protected void saveSelectionDetail()
	{
		...
		//
		int LVE_OrderPrePaySchedule_ID = 0;
		int row = p_table.getSelectedRow();
		if (row >= 0)
		{
			Object value = p_table.getValueAt(row, INDEX_PAYSCHEDULE);
			if (value != null && value instanceof KeyNamePair)
				LVE_OrderPrePaySchedule_ID = ((KeyNamePair)value).getKey();
		}
		if (LVE_OrderPrePaySchedule_ID <= 0)	//	not selected
			Env.setContext(Env.getCtx(), p_WindowNo, Env.TAB_INFO, "LVE_OrderPrePaySchedule_ID", "0");
		else
			Env.setContext(Env.getCtx(), p_WindowNo, Env.TAB_INFO, "LVE_OrderPrePaySchedule_ID", String.valueOf(LVE_OrderPrePaySchedule_ID));
	}	//	saveSelectionDetail
	private static int INDEX_PAYSCHEDULE = 0;	//	last item
}   //  InfoOrder
