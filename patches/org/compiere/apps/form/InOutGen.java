...
public class InOutGen extends GenForm
{
	...
	public void configureMiniTable(IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(7, Boolean.class, true, Msg.translate(Env.getCtx(), "IsImmediateDelivery"));
		//
		miniTable.autoSize();
	}
	...
	//  Create SQL
        StringBuffer sql = new StringBuffer(
            "SELECT C_Order_ID, o.Name, dt.Name, DocumentNo, bp.Name, DateOrdered, TotalLines, ic.IsImmediateDelivery "
	...
	private String getRMASql()
	{
	    ...
	    sql.append("SELECT rma.M_RMA_ID, org.Name, dt.Name, rma.DocumentNo, bp.Name, rma.Created, rma.Amt, 'Y' IsImmediateDelivery ");
	...
	public void executeQuery(KeyNamePair docTypeKNPair, IMiniTable miniTable)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				String m_IsImmediateDelivery = rs.getString(8);
				miniTable.setValueAt((m_IsImmediateDelivery != null 
						&& m_IsImmediateDelivery.equals("Y")), row, 7);		//	IsImmediateDelivery
				//  prepare next
				row++;
			}
			rs.close();
