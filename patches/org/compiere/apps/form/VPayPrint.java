...
public class VPayPrint extends PayPrint implements FormPanel, ActionListener, VetoableChangeListener
{
	...
	private void dynInit() throws Exception
	{
		...
		//	Yamel Senih 05-11-2013
		// 	Reviewed and updated to add validation to show only pending payments
		/*MLookup lookupPS = MLookupFactory.get(Env.getCtx(), m_WindowNo, 0, DisplayType.TableDir, Env.getLanguage(Env.getCtx()), 
				"C_PaySelection_ID", 0, false, "EXISTS(SELECT 1 FROM C_PaySelection ps " +
						"										INNER JOIN C_PaySelectionCheck psc ON(psc.C_PaySelection_ID = ps.C_PaySelection_ID) " +
						"									WHERE ps.C_PaySelection_ID = C_PaySelection.C_PaySelection_ID " +
						"									AND ps.Processed='Y' AND ps.IsActive='Y' AND psc.C_Payment_ID IS NULL)");*/
		
		
		String sqlExists = " EXISTS(SELECT 1 FROM C_PaySelection ps"
				+ " INNER JOIN C_PaySelectionCheck psc ON(psc.C_PaySelection_ID = ps.C_PaySelection_ID)"
				+ " LEFT JOIN C_PaySelectionLine psl ON(psl.C_PaySelection_ID = ps.C_PaySelection_ID)"
				+ "	LEFT JOIN LVE_PaymentRequestLine prl ON (psl.LVE_PaymentRequestLine_ID = prl.LVE_PaymentRequestLine_ID)"
				+ " LEFT JOIN LVE_PaymentRequest pr ON (prl.LVE_PaymentRequest_ID = pr.LVE_PaymentRequest_ID)"
				+ " WHERE"
				+ "		ps.C_PaySelection_ID = C_PaySelection.C_PaySelection_ID"
				+ "		AND ps.Processed='Y' "
				+ "		AND ps.IsActive='Y'"
				+ "		AND psc.C_Payment_ID IS NULL"
				+ "		AND (pr.DocStatus = 'CO' OR pr.DocStatus IS NULL))";
				
		MLookup lookupPS = MLookupFactory.get(Env.getCtx(), m_WindowNo, 0, DisplayType.TableDir, Env.getLanguage(Env.getCtx()), 
				"C_PaySelection_ID", 0, false, sqlExists);
	...
	private void cmd_export()
	{
		...
		if(m_checks != null && m_checks.length != 0)
			m_batch = new MPaymentBatch(Env.getCtx(), m_checks[0].getC_Payment().getC_PaymentBatch_ID(), trx.getTrxName());
		...
		//	
		Env.setContext(Env.getCtx(), "#CurrentPath", fc.getCurrentDirectory().getPath());
		...
		try
		{
			...
			no = custom.exportToFile(m_checks, new File(pathName), err);
		}
	...
	//	{
		...
		//}	//	confirm
			...
			//	Yamel Senih 05-11-2013			
			//	Coment for not impress Remittance
		/*if (ADialog.ask(m_WindowNo, panel, "VPayPrintPrintRemittance"))
		{
			...
			//	Yamel Senih 05-11-2013		
			//	Comment for not impress Remittance

		boolean somethingPrinted = false;
		boolean directPrint = !Ini.isPropertyBool(Ini.P_PRINTPREVIEW);
		//	Valid Payment Request
		int m_C_PaySelectionLine_ID = DB.getSQLValue(null, "SELECT l.C_PaySelectionLine_ID "
				+ "FROM C_PaySelectionLine l "
				+ "WHERE l.C_PaySelectionCheck_ID = ? "
				+ "AND l.LVE_PaymentRequest_ID IS NOT NULL", m_checks[0].getC_PaySelectionCheck_ID());
		
		if(m_C_PaySelectionLine_ID <= 0) {
			for (int i = 0; i < m_checks.length; i++)
			{
				MPaySelectionCheck check = m_checks[i];
			...
			for (int i = m_checks.length -1 ; i >= 0 ; i--)
			{
				MPaySelectionCheck check = m_checks[i];
				//	ReportCtrl will check BankAccountDoc for PrintFormat
				boolean ok = ReportCtl.startDocumentPrint(ReportEngine.CHECK, check.get_ID(), null, Env.getWindowNo(panel), false);
				if (!somethingPrinted && ok)
					somethingPrinted = true;
			}
		}
	...
	private boolean getChecks(String PaymentRule)
	{
		...
		MPaySelection m_PaySelection = new MPaySelection(Env.getCtx(), m_C_PaySelection_ID, null);
		m_batch.setProcessingDate((Timestamp) m_PaySelection.getPayDate());
		return true;
	}   //  getChecks
