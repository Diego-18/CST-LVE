 ...
 *  	<li> Fixed columns increment in minitable
 *  	<li> Fixed concepts increment in fieldConcept
 */
public class VHRActionNotice extends HRActionNotice implements FormPanel, ActionListener, VetoableChangeListener
{
	...
	//	Yamel Senih 2014-06-30, 14:10:39
	//	Add Lookup
	private static VComboBox 	fieldTextMsgLookup = new VComboBox();
	private static boolean 	isLookupTextMsg = false;
	//	
	/**	Window No			*/
	...
	private void jbInit()
	{
		...
		parameterPanel.add(fieldText,  new GridBagConstraints(3, 2, 1, 1, 0.0, 0.0
				,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 0, 0));
		//	Field Text Msg Lookup
		parameterPanel.add(fieldTextMsgLookup,  new GridBagConstraints(3, 2, 1, 1, 0.0, 0.0
				,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 0, 0));
		//	
		// Description
	...
	public void dynInit() throws Exception
	{
		...
		fieldTextMsgLookup.setVisible(false);
	 ...
	 * @author <a href="mailto:yamelsenih@gmail.com">Yamel Senih</a> 30/06/2014, 14:12:42
	 * @param m_AD_Reference_Value_ID
	 * @return void
	 */
	private void loadTextMsgLookup(int m_AD_Reference_Value_ID) {
		//	Remove all Items
		fieldTextMsgLookup.removeAllItems();
		...
		//	Get Current Concept
		//	Valid reference
		if(m_AD_Reference_Value_ID == 0)
			return;
		//	Set to new
		ArrayList<ValueNamePair> conceptData = getConceptReference(m_AD_Reference_Value_ID);
		for(ValueNamePair vp : conceptData) {
			fieldTextMsgLookup.addItem(vp);
		}
		//	Set Flag
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if ( e.getSource().equals(fieldProcess) ) {					// Process
			...
			if ( m_C_BPartner_ID > 0){			
				...
				//	Yamel Senih 2013-03-11
				//	Fixed concepts increment in fieldConcept
			...
			if (m_HR_Concept_ID > 0) {
				...
				if (sHR_Movement_ID > 0){
					...
					if ( concept.getColumnType().equals(MHRConcept.COLUMNTYPE_Quantity) )	// Quantity
						...
						if(isLookupTextMsg)
							fieldTextMsgLookup.setValue(movementFound.getTextMsg());
						else
				...
				if (concept.getColumnType().equals(MHRConcept.COLUMNTYPE_Quantity)){		// Concept Type
					...
					fieldRuleE.setVisible(false);
					//	
					if(concept.getAD_Reference_ID() != 0) {
						fieldText.setVisible(false);
						fieldText.setReadWrite(false);
						fieldTextMsgLookup.setVisible(true);
						fieldTextMsgLookup.setReadWrite(true);
					} else {
						fieldText.setVisible(true);
						fieldText.setReadWrite(true);
						fieldTextMsgLookup.setVisible(false);
						fieldTextMsgLookup.setReadWrite(false);
					}
				}
	...
	public static void saveMovement()
	{
		...
		String textMsg = null;
		if(isLookupTextMsg)
			textMsg = (String)fieldTextMsgLookup.getValue();
		else
			textMsg = fieldText.getValue() != null ? (String)fieldText.getValue().toString() : "";
		//	
		movementOK.setTextMsg(textMsg);
		...
		MHREmployee employee  = MHREmployee.getActiveEmployee(Env.getCtx(), movementOK.getC_BPartner_ID(), m_process.getAD_Org_ID(), null);
		if (employee != null) {
		...
		//	Yamel Senih 2014-04-07, 16:22:37 Add Is Printed
		movementOK.setIsPrinted(conceptOK.isPrinted());
		//	End
		movementOK.saveEx();
