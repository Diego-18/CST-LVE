...
public class HRActionNotice
{
	...
	public void configureMiniTable(IMiniTable miniTable)
	{
		...
		//  Raul Muñoz 19/02/2015 16:00 repaint column table.
		miniTable.repaint();
		miniTable.autoSize();
	}
	...
	public static List<KeyNamePair> getEmployeeValid(MHRProcess process)
	{
		...
		list.add(pp);
		String sql = MRole.getDefault().addAccessSQL(
				"SELECT DISTINCT bp.C_BPartner_ID, (bp.Value || ' - ' || bp.Name || COALESCE(' ' || bp.Name2, '')) BPName " +
				"FROM HR_Employee hrpe " +
				"INNER JOIN C_BPartner bp ON(bp.C_BPartner_ID=hrpe.C_BPartner_ID)",
				"hrpe",MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO) + " AND hrpe.IsActive = 'Y' AND hrpe.AD_Org_ID = " + process.getAD_Org_ID();
		if ( process.getHR_Payroll_ID() != 0){
			//	Yamel Senih 2014-05-05, 22:20:10
			//	Valid Employee Valid
			boolean isNotRegularPayroll = MHRPayroll.get(Env.getCtx(), process.getHR_Payroll_ID())
																	.get_ValueAsBoolean("IsNotRegularPayroll");
			//	End Yamel Senih
			...
			if ( process.getHR_Department_ID() > 0 )
 			...
 			if ( process.getC_BPartner_ID() > 0 )
 				sql += " AND hrpe.C_BPartner_ID =" + process.getC_BPartner_ID();
		}
		sql += " ORDER BY BPName ";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	 ...
	 * @author <a href="mailto:yamelsenih@gmail.com">Yamel Senih</a> 30/06/2014, 16:35:36
	 * @param m_AD_Reference_Value_ID
	 * @return
	 * @return ArrayList<ValueNamePair>
	 */
	public ArrayList<ValueNamePair> getConceptReference(int m_AD_Reference_Value_ID) {
		ArrayList<ValueNamePair> data = new ArrayList<ValueNamePair>();

		//	Get Lookup Info
		MLookupInfo infoLookup = MLookupFactory.getLookup_List(Env.getLanguage(Env.getCtx()), m_AD_Reference_Value_ID);
		//	Valid Lookup
		if(infoLookup == null)
			return data;
		//	Get SQL
