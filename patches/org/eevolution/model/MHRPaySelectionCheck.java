...
public final class MHRPaySelectionCheck extends X_HR_PaySelectionCheck
{
		...
		String PaymentRule, int startDocumentNo, String trxName)
	{
		...
		//	Yamel Senih 2014-08-20, 20:32:47
		//	Add Support to Voided Check
		//	Old Query
		...
		final String where = I_HR_PaySelectionCheck.COLUMNNAME_HR_PaySelection_ID + "=? AND "
							   + I_HR_PaySelectionCheck.COLUMNNAME_PaymentRule + "=? "
							   + " AND " +
							   "(" + I_HR_PaySelectionCheck.COLUMNNAME_C_Payment_ID + " IS NULL" +
							   		" OR EXISTS(SELECT 1 " +
							   		"			FROM C_Payment p WHERE p.C_Payment_ID = HR_PaySelectionCheck.C_Payment_ID" +
							   				" AND p.DocStatus IN('VO', 'RE')" +
							   				")" +
							   	")";
	...
	public static int confirmPrint (Collection<MHRPaySelectionCheck> checks , MPaymentBatch batch)
	{
		...
		{
			//	Yamel Senih 2014-08-14, 11:38:17
			//	best Query
			String sqlConcept = "SELECT HR_Concept_ID FROM HR_Movement m " +
									" WHERE m.C_BPartner_ID = " + check.getC_BPartner_ID() +
									" AND EXISTS(SELECT 1 FROM HR_PaySelectionLine psl " +
									"						WHERE psl.HR_Movement_ID = m.HR_Movement_ID " +
									" 						AND HR_PaySelection_ID = ?)";	
			s_log.info("Concept: " + sqlConcept);
			
			int HR_Concept_ID = DB.getSQLValue(check.get_TrxName(), sqlConcept, check.getHR_PaySelection_ID());
			
			String sqlPayroll = "SELECT p.HR_Payroll_ID FROM HR_Process p " +
									" WHERE EXISTS(SELECT 1 FROM HR_Movement m " +
									" 					WHERE m.HR_Process_ID = p.HR_Process_ID " +
									" 					AND m.C_BPartner_ID = " + check.getC_BPartner_ID() +
									"						AND EXISTS(SELECT 1 FROM HR_PaySelectionLine psl " +
									"							WHERE psl.HR_Movement_ID = m.HR_Movement_ID " +
									" 							AND HR_PaySelection_ID = ?))";
			
			s_log.info("Payroll: " + sqlPayroll);
			int HR_Payroll_ID = DB.getSQLValue(check.get_TrxName(), sqlPayroll, check.getHR_PaySelection_ID());
			...
			//else	//	New Payment
			if(check.getC_Payment_ID() == 0
					|| payment.getDocStatus().equals(X_C_Payment.DOCSTATUS_Voided)
				...
				else if (check.getPaymentRule().equals(PAYMENTRULE_DirectDeposit)		//	Yamel Senih 2014-08-20, 19:46:34 Add support to ACH	
					|| check.getPaymentRule().equals(PAYMENTRULE_DirectDebit)) {
					payment.setBankACH(check.getParent().getC_BankAccount_ID(), false, 
							X_C_Payment.TENDERTYPE_DirectDeposit, null, null);
				}																		//	End Yamel Senih
				else
				{
				...
				//	Set Payment Selection Description
				MHRPaySelection paySelection = (MHRPaySelection) check.getHR_PaySelection();
				//	Set payment date from selection check
				...
				if(paySelection != null)
					currentDate = paySelection.getPayDate();
				//	Set Date
				payment.setDateTrx(currentDate);
				payment.setDateAcct(currentDate); // globalqss [ 2030685 ]
				payment.setC_BPartner_ID(check.getC_BPartner_ID());
				//	Set value Payroll Payment
				payment.set_ValueOfColumn("IsPayrollPayment", true);
				//	Valid Null
				if(paySelection != null)
					payment.setDescription(paySelection.getDescription());
				//	Link to Batch
				if (batch != null)
				{
