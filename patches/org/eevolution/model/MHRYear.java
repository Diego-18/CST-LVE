		...
		for (int period = 1; period <= getQty(); period++)
		{
			...
			if ((12 == getQty()) && (28 == getNetDays() || 29 == getNetDays() || 30 == getNetDays() || 31 == getNetDays()))
			{
				...
				if(day == 16) {
					...
					//	Verify if is not 15 Days
					if(cal.get(Calendar.DAY_OF_MONTH) != 15)
						EndDate = TimeUtil.getDay(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH) + 1, 15);
				} else
					EndDate = TimeUtil.getMonthLastDay(StartDate);
				
			}
			//	Yamel Senih 2013-12-13 09:11:14
			//	Add Support to fortnight
			else if (24 == getQty() && 15 == getNetDays())
			{
				if (period >1) {
					StartDate = TimeUtil.addDays(EndDate, 1);
				} else {
			...
			HR_Period.setName(format.format(StartDate)+" "+Msg.translate(getCtx(), "To")+" "+format.format(EndDate));
			HR_Period.setDescription(Msg.translate(getCtx(), "HR_Payroll_ID")+" "+payroll.getName().trim()+" "+Msg.translate(getCtx(), "From")+ " "+period+" " +Msg.translate(getCtx(), "To")+" "+ format.format(StartDate)+" al "+format.format(EndDate));
