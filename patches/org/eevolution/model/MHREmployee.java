...
public class MHREmployee extends X_HR_Employee
{
	...
	public static MBPartner[] getEmployees (MHRProcess p)
	{
		...
		MHRPayroll payroll = MHRPayroll.get(p.getCtx(), p.getHR_Payroll_ID());
		// This payroll not content periods, NOT IS a Regular Payroll > ogi-cd 28Nov2007
		if(p.getHR_Payroll_ID() != 0 
				&& p.getHR_Period_ID() != 0
	...
	/**
	 * Active Employee
	 * @author <a href="mailto:yamelsenih@gmail.com">Yamel Senih</a> 12/09/2014, 14:43:49
	 * @param ctx
	 * @param C_BPartner_ID
	 * @param AD_Org_ID
	 * @param trxName
	 ...
	 * @return MHREmployee
	 */
	public static MHREmployee getActiveEmployee(Properties ctx, int C_BPartner_ID, int AD_Org_ID, String trxName) {
		return new Query(ctx, Table_Name, COLUMNNAME_C_BPartner_ID+"=? AND " + COLUMNNAME_AD_Org_ID+"=?", trxName)
							.setOnlyActiveRecords(true)
							.setParameters(new Object[]{C_BPartner_ID, AD_Org_ID})
