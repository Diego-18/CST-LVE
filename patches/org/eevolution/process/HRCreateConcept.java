...
public class HRCreateConcept extends SvrProcess
{
	...
	//	Yamel Senih 2014-04-22, 23:21:27
	//	Add Concept Category Filter
	/**	Concept Category		*/
	...
	protected void prepare ()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				;
			else if (name.equals("HR_Concept_Category_ID"))
				p_HR_Concept_Category_ID = para[i].getParameterAsInt();
			else
	...
	{
		int count = 0;
		String sqlWhere = (p_HR_Concept_Category_ID != 0? 
				" AND HR_Concept.HR_Concept_Category_ID = " + p_HR_Concept_Category_ID:
					null);
		for(MHRConcept concept : MHRConcept.getConcepts(p_HR_Payroll_ID, 0, sqlWhere))
		{
			if (concept.isPaid()
