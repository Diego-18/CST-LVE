...
public class MLVEOrderPrePaySchedule extends X_LVE_OrderPrePaySchedule implements
	...
	public MLVEOrderPrePaySchedule (MOrder order, MPaySchedule paySchedule)
	{
		...
		MPaymentTerm pt = new MPaymentTerm(getCtx(), paySchedule.getC_PaymentTerm_ID(), get_TrxName());
		//	Amounts
		int scale = MCurrency.getStdPrecision(getCtx(), order.getC_Currency_ID());
		BigDecimal due = pt.get_ValueAsBoolean("IsLineAmtBased") ? order.getTotalLines() : order.getGrandTotal() ;
	...
	public boolean validatePaySchedule()
	{
		...
		/** 2014-11-20 Carlos Parada Set Valid Based in Grand Total of Document */
		/*
		MPaymentTerm pt = new MPaymentTerm(getCtx(), m_parent.getC_PaymentTerm_ID(), get_TrxName());
		boolean isLineAmtBased = pt.get_ValueAsBoolean("IsLineAmtBased");
		BigDecimal amount = isLineAmtBased ? m_parent.getTotalLines()  : m_parent.getGrandTotal() ;
