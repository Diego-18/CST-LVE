...
public class OrderPrePayScheduleValidate extends SvrProcess
{
	...
	{
		log.info ("LVE_OrderPrePaySchedule_ID=" + getRecord_ID());
		...
		//
		remainder = pt.get_ValueAsBoolean("IsLineAmtBased") ? order.getTotalLines() : order.getGrandTotal() ;
		//	End Dixon Martinez
			
		boolean valid = remainder.compareTo(total) == 0;
		...
		String msg = "@OK@";
		if (!valid){
			msg = pt.get_ValueAsBoolean("IsLineAmtBased") ? "@TotalLines@ = " :  "@GrandTotal@ = " ;
			msg +=  remainder 
				+ " <> @Total@ = " + total 
